...
(async () => {
  ...
  try {
    ...
    const selectSql = 'SELECT sq_id, sq_bc FROM squeegees WHERE sq_qr IS NULL AND sq_bc IS NOT NULL;';
    ...
    for (const record of recordsToFix) {
      ...
      console.log(` -> Procesando Squeegee ID: ${record.sq_id} con Barcode: ${record.sq_bc}`);
      ...
      const qrBuffer = await qrcode.toBuffer(record.sq_bc);
      ...
      const updateSql = 'UPDATE squeegees SET sq_qr = $1 WHERE sq_id = $2;';

      // CORREGIDO: Se usa record.sq_id como parámetro
      await pool.query(updateSql, [qrBuffer, record.sq_id]);
    }
