...
(async () => {
  ...
  try {
    ...
    const selectSql = 'SELECT st_id, st_bc FROM stencils WHERE st_qr IS NULL AND st_bc IS NOT NULL;';
    ...
    for (const record of recordsToFix) {
      ...
      console.log(` -> Procesando Squeegee ID: ${record.st_id} con Barcode: ${record.st_bc}`);
      ...
      const qrBuffer = await qrcode.toBuffer(record.st_bc);
      ...
      const updateSql = 'UPDATE stencils SET st_qr = $1 WHERE st_id = $2;';

      // CORREGIDO: Se usa record.sq_id como parámetro
      await pool.query(updateSql, [qrBuffer, record.st_id]);
    }
