...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadAllTools() {
    toolListContainer.innerHTML =
      '<div class="text-center p-5">Cargando herramientas...</div>';
    try {
      ...
      allTools = [
        ...stencils.map((s) => ({
          ...s,
          type: "stencil",
          displayId: `st-${s.st_id}`,
          id: s.st_id,
          name: `${s.st_job || ""} ${s.pn_pcb || ""} ${s.st_bc || ""}`,
        })),
        ...plates.map((p) => ({
          ...p,
          type: "plate",
          displayId: `pl-${p.pl_id}`,
          id: p.pl_id,
          name: `${p.pl_job || ""} ${p.pn_pcb || ""} ${p.pl_bc || ""}`,
        })),
        ...squeegees.map((sq) => ({
          ...sq,
          type: "squeegee",
          displayId: `sq-${sq.sq_id}`,
          id: sq.sq_id,
          name: `${sq.sq_length || ""}mm ${sq.sq_side || ""} ${sq.sq_bc || ""}`,
        })),
      ];
  ...
  function displayTools(toolsToDisplay) {
    ...
    if (toolsToDisplay.length === 0) {
      toolListContainer.innerHTML =
        '<div class="text-center p-5">No se encontraron herramientas.</div>';
      return;
    }
    ...
    toolsToDisplay.forEach((tool) => {
      ...
      switch (tool.type) {
        case "stencil":
          labelText = `Stencil #${tool.id}: ${tool.st_job || "N/A"} (PCB: ${
            tool.pn_pcb || "N/A"
          }, ${tool.st_side || "N/A"}, ${tool.thickness || "N/A"}mm) - ${
            tool.st_bc || "No Barcode"
          }`;
          break;
        case "plate":
          labelText = `Base #${tool.id}: ${tool.pl_job || "N/A"} (PCB: ${
            tool.pn_pcb || "N/A"
          }, Prov: ${tool.supp_name || "N/A"}) - ${tool.pl_bc || "No Barcode"}`;
          break;
        case "squeegee":
          labelText = `Squeegee #${tool.id}: ${
            tool.sq_length || "N/A"
          }mm Lado ${tool.sq_side || "N/A"} - ${tool.sq_bc || "No Barcode"}`;
          break;
        default:
